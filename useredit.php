...
?>
		...
		<div class="hero-unit"><!--HERO -->	
			...
			<form name= "taskedit" class="form-vertical" method="POST" action= "updatetask.php" onsubmit="return checkform(start.value,end.value,project.value);">
				<br />
				<label><b>Client</b></label>
				<input type="hidden" value="<?php echo $result['pid']; ?>" name="pid"/>
				<input type="text" name="client" autocomplete="off" value="<?php echo $result['client'];?>"data-provide="typeahead" data-items="3" onchange="getpro(this.value,pid.value)" data-source='[<?php 
                                  $len=count($res1);
                                  for($i=0;$i<$len;$i++)
                                  {
                                      echo '"'.$res1[$i]['name'].'"';
				...
				<a href = "userhome.php?dt=<?php echo $result['t_date']; ?>" class = "btn btn-danger">Cancel</a>
			</form></div>
			</div> <!-- /span-row2 -->
