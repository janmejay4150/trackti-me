...
if(isset($_GET['msg5']))
{
	...
	if($a=="err")
		{	
			...
			echo "<h4>Customer already exists</h4>";
			echo "</div>";
		}
}
?>
...
<form class="form-horizontal" name="addproj" method="POST" action="addpro.php">
	<p>
		...
		<?php
			...
			echo "<option value='#'>Existing Client</option>";
			foreach($var as $name)
			{
...
<form class="form-inline" name="assgproj" method="POST" action="checkassg.php"  onsubmit = "return validation()">
	...
	<input type="text" name="client" autocomplete="off" data-provide="typeahead" data-items="3" onchange="getpro(this.value)" data-source='[<?php 
   	...
   	<p><h5>Projects:</h5>
	<?php
		echo "<select name='project' id='allproject'>";
		echo "</select>";
	?></p><br />
    <p><h5>Employees:</h5>
	<?php
		...
		$where1= "status=1 ORDER BY username";
...
<div class="span3">
					...
					<form name="bydate" method="POST" action="adminhome.php" onsubmit="return ValidateForm();">
						<p><input name="dt"  autocomplete="off" value="<?php echo $setdate;?>" type="text" id="datepicker" required = "required"/></p>
						<h5>Name:</h5>
						<p>
							<?php
								$db= new Database();
